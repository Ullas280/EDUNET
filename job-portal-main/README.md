...
## 🖥️ Backend
- **Django (Python)** – Server-side framework for APIs (project in `server_django/`)
- **MySQL** – Relational database used by the Django backend
- **Django REST Framework** – API and serialization layer
- **Simple JWT** – JWT-based authentication for the Django backend

Note: The original Node/Express + MongoDB backend has been archived to `archive/server_node_backup_20251116_120000/`.
The active backend is now the Django project located at `server_django/`.
